...
PanelLayout::PanelLayout(Params params, PanelLayoutType type)
        : params(params), layout_type(type), current_panel_index(0), current_panel_position(0) {}

PanelLayout::~PanelLayout() { }

const Params& PanelLayout::get_params() const { return params; }


PanelLayoutType PanelLayout::get_layout_type() const { return layout_type; }
    
void PanelLayout::set_layout_type(PanelLayoutType type) { layout_type = type; }

int PanelLayout::get_layout_height(float zoom_level) {
...
const Panel& PanelLayout::get_panel(int index) const { 
    assert(index < (int)panels.size());
    return panels[(size_t)index]; 
}

void PanelLayout::reset_panels(std::vector<Panel> panels) {
    this->panels.clear();
...
int PanelLayout::calculate_pos_for_current_panel_info(float zoom_factor) {
...
void PanelLayout::set_current_panel_from_scroll_pos(int scroll_pos, float zoom_level) {
...
void PanelLayout::set_current_panel(int index, int position) {
    current_panel_index = index;
    current_panel_position = position;
}

int PanelLayout::get_current_panel_index() const { return current_panel_index; }
int PanelLayout::get_current_panel_position() const { return current_panel_position; }



uint32_t PanelLayout::get_vertical_slot_offset() const {
