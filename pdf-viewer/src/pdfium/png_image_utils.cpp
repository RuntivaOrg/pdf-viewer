...
void DecodeInfoCallback(png_struct* png_ptr, png_info* info_ptr) {
  ...
  state->output->resize(static_cast<size_t>(state->width * state->output_channels * state->height));
}
...
void DecodeRowCallback(png_struct* png_ptr,
  ...
  uint8_t* dest = &base[static_cast<unsigned int>(state->width * state->output_channels) * row_num];
  if (state->row_converter)
    state->row_converter(new_row, state->width, dest, &state->is_opaque);
  else
    memcpy(dest, new_row, static_cast<size_t>(state->width * state->output_channels));
}
...
bool DoLibpngWrite(png_struct* png_ptr,
  ...
  png_set_IHDR(png_ptr, info_ptr, static_cast<png_uint_32>(width), static_cast<png_uint_32>(height), 8, png_output_color_type,
  ...
  if (!converter) {
    ...
    for (int y = 0; y < height; y++) {
      png_write_row(png_ptr, const_cast<uint8_t*>(&input[static_cast<size_t>(y * row_byte_width)]));
    }
  } else {
    // Needs conversion using a separate buffer.
    row_buffer = new uint8_t[static_cast<size_t>(width * output_color_components)];
    for (int y = 0; y < height; y++) {
      converter(&input[static_cast<size_t>(y * row_byte_width)], width, row_buffer, nullptr);
