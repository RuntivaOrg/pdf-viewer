...
static std::vector<int> calculateDimensions(std::vector<std::pair<int, int>> minMax, int dimension)
{
  ...
  {
      int val = static_cast<int>((i + 1) * static_cast<size_t>(dimension) / minMax.size() -
                                 i * static_cast<size_t>(dimension) / minMax.size());
      result.push_back(val);//static_cast<int>((i + 1) * static_cast<size_t>(dimension) / minMax.size() -
                              //          i * static_cast<size_t>(dimension) / minMax.size()));
    untouchableList.push_back(false);
  }
  auto touchableCount = static_cast<int>(minMax.size());
